  ...
  app.patch("/api/invoices/:id/status", (req: Request, res: Response) => {
    try {
      ...
      const totalPaid = verifiedPayments.reduce((sum: number, p: any) => {
        ...
        const alloc = p.invoices.find((ai: any) => (ai.id === existingInvoice.id || ai.invoiceId === existingInvoice.id));
        return sum + Number(alloc.amountApplied || alloc.paymentAmount || 0);
      }, 0);
    ...
    try {
      ...
      if (status === 'Verified' || status === 'Received' || status === 'PAID' || status === 'PAID_SUCCESS') {
        ...
        try {
          ...
          // Helper to update a single invoice based on all verified payments
          const updateInvoiceBalance = (invId: string) => {
            const invoiceIndex = invoicesData.invoices.findIndex((i: any) => i.id === invId);
            if (invoiceIndex !== -1) {
              const invoice = invoicesData.invoices[invoiceIndex];
              
              // Recalculate based on ALL verified payments in the system for this invoice
              // Check both single link and multi-allocation
              const allPayments = paymentsReceivedData.paymentsReceived.filter((p: any) => 
                p.invoiceId === invoice.id || (p.invoices && p.invoices.some((ai: any) => (ai.id === invoice.id || ai.invoiceId === invoice.id)))
              );
              ...
              const totalPaid = verifiedPayments.reduce((sum: number, p: any) => {
                if (p.invoiceId === invoice.id) return sum + Number(p.amount || 0);
                const alloc = p.invoices?.find((ai: any) => (ai.id === invoice.id || ai.invoiceId === invoice.id));
                return sum + Number(alloc?.amountApplied || alloc?.paymentAmount || 0);
              }, 0);

              invoice.amountPaid = totalPaid;
              invoice.balanceDue = Math.max(0, Number(invoice.total || invoice.amount || 0) - totalPaid);
              
              if (invoice.balanceDue <= 0 && Number(invoice.total || invoice.amount || 0) > 0) {
                invoice.status = 'Paid';
              } else if (totalPaid > 0) {
          ...
          if (payment.invoiceId) {
            updateInvoiceBalance(payment.invoiceId);
          }
          
          if (payment.invoices && payment.invoices.length > 0) {
            payment.invoices.forEach((invAlloc: any) => {
              updateInvoiceBalance(invAlloc.id || invAlloc.invoiceId);
            });
          }
