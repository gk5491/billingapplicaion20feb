...
export default function VendorProfilePage() {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    if (data.success && data.data) {
                        setVendor(data.data.vendor || data.data);
                        if (data.data.transactions) {
                            setTransactions((prev: any) => ({ ...prev, ...data.data.transactions }));
                        }
    ...
    const fetchVendorData = async () => {
        if (!vendor?.id) return;
        try {
            const [commentsRes, transactionsRes, mailsRes, activitiesRes, billsRes] = await Promise.all([
            ...
            if (transactionsRes.ok) {
                ...
                setTransactions((prev: any) => ({ ...prev, ...data.data }));
            }

            if (billsRes.ok) {
                const billsData = await billsRes.json();
                const filteredBills = (billsData.data || []).filter((bill: any) => String(bill.vendorId) === String(vendor.id));
                setTransactions((prev: any) => ({ ...prev, bills: filteredBills }));
            }
    ...
    const formatAddress = (address: any) => {
        if (!address) return ['-'];
        const parts = [address.street1 || address.street, address.street2, address.city, address.state, address.country, address.pinCode || address.pincode].filter(Boolean);
    ...
    const getExpenseData = () => {
        const now = new Date();
        const monthsToInclude = expensePeriod === 'last-6-months' ? 6 : 12;
        ...
        const txList = (transactions.bills || []).filter((bill: any) => bill.status !== 'Void');

        txList.forEach((tx: any) => {
            const txDate = parseISO(tx.billDate || tx.date);
            const mStart = startOfMonth(txDate);
            const monthIdx = months.findIndex(m => m.date.getTime() === mStart.getTime());
            if (monthIdx !== -1) {
                months[monthIdx].expense += tx.total || tx.amount || 0;
            }
        });
    ...
    const statementTransactions = transactions.bills || [];
        ...
        <div className="h-full flex flex-col bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-slate-700">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-sidebar-accent/5">
                <div className="flex items-center gap-3">
                    <h2 className="text-xl font-semibold text-sidebar font-display truncate" data-testid="text-vendor-name">{vendor.displayName || vendor.companyName || vendor.name}</h2>
                </div>
                <div className="flex items-center gap-2">
                    <Button variant="outline" size="sm" onClick={() => setLocation("/vendor/settings/profile")} data-testid="button-edit-vendor">
                        Edit
                    </Button>
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-56">
                            ...
                            <DropdownMenuItem onClick={() => setLocation("/purchase-orders/new")}>
                                <Package className="mr-2 h-4 w-4" /> Purchase Order
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => setLocation("/bills/new")}>
                                <Receipt className="mr-2 h-4 w-4" /> Bill
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => setLocation("/payments-made/new")}>
                ...
                <div className="flex-1 overflow-y-auto scrollbar-hide">
                        ...
                        <div className="flex h-full">
                            <div className="w-72 border-r border-slate-200 dark:border-slate-700 p-6 overflow-auto scrollbar-hide">
                                <div className="space-y-6">
                                    <div>
                                        <h3 className="text-lg font-semibold text-slate-900 dark:text-white" data-testid="text-vendor-display-name">{vendor.displayName || vendor.companyName || vendor.name}</h3>
                                        {vendor.email && (
                                            <p className="text-sm text-sidebar font-medium mt-2 font-display" data-testid="text-vendor-email">{vendor.email}</p>
                                        )}
                                    </div>
                                        ...
                                        <CollapsibleContent className="mt-3 space-y-4">
                                            <div className="mt-4">
                                                ...
                                                <div className="text-sm">
                                                    {formatAddress(vendor.billingAddress).map((line: any, i: any) => <p key={i}>{line}</p>)}
                                                </div>
                                            </div>
                                            <div className="mt-4">
                                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1 font-display">Shipping Address</p>
                                                <div className="text-sm">
                                                    {formatAddress(vendor.shippingAddress).map((line: any, i: any) => <p key={i}>{line}</p>)}
                                                </div>
                                            </div>
                                        </CollapsibleContent>
                                    </Collapsible>
                            ...
                            <div className="flex-1 overflow-auto scrollbar-hide">
                                <div className="w-full">
                                    ...
                                    <div className="mb-6 mx-6">
                                        ...
                                        <p className="text-sm font-semibold text-slate-700 font-display">{vendor.paymentTerms || 'Due on Receipt'}</p>
                                    </div>
                                            ...
                                            <table className="w-full text-sm">
                                                ...
                                                <tbody>
                                                    <tr className="group hover:bg-slate-50 transition-colors">
                                                        ...
                                                        <td className="px-4 py-3 text-right font-semibold text-sidebar font-display">{formatCurrency(vendor.balanceToPay || vendor.outstandingPayables || 0)}</td>
                                                        <td className="px-4 py-3 text-right font-semibold text-green-600 font-display">{formatCurrency(vendor.unusedCredits || 0)}</td>
                                                    </tr>
                                                </tbody>
                                            </table>
                                        </div>
                        ...
                        <div className="w-full p-6 max-w-4xl mx-auto">
                            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm overflow-hidden mb-8 transition-all focus-within:ring-2 focus-within:ring-blue-500/20 focus-within:border-blue-500">
                                ...
                                <div className="p-3 bg-slate-50 border-t border-slate-200 flex justify-end">
                                    <Button onClick={handleAddComment} disabled={!newComment.trim() || isAddingComment} size="sm" className="bg-sidebar hover:bg-sidebar/90 text-white font-bold font-display px-6 shadow-sm" data-testid="button-add-comment">
                                            ...
                                            <div className="flex justify-between items-start mb-2">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-[10px] text-slate-400 font-display">{format(new Date(comment.createdAt || comment.date), "MMM d, yyyy h:mm a")}</p>
                                                    </div>
                                                </div>
                                            </div>
                                            <p className="text-sm text-slate-600 dark:text-slate-400 mt-2 font-display leading-relaxed whitespace-pre-wrap">{comment.text || comment.content}</p>
                                        </div>
                                    ))}
                                </div>
                            )}
                        ...
                        <div className="mx-auto max-w-6xl space-y-8">
                            {Object.entries(transactions).map(([type, list]: [string, any]) => (
                                <div key={type} className="space-y-4">
                                    <h4 className="font-bold text-sidebar font-display capitalize">{type.replace(/([A-Z])/g, ' $1').trim()}</h4>
                                    <Table>
                    ...
                    <TabsContent value="statement" className="flex-1 overflow-y-auto scrollbar-hide p-0 mt-0">
                        <div className="h-full overflow-auto scrollbar-hide p-8 flex flex-col items-center bg-slate-100">
                            ...
                            <div className="bg-white text-slate-900 shadow-xl px-10 py-10 w-full max-w-[210mm] min-h-[296mm] h-fit" id="vendor-statement" style={{ color: '#000000' }}>
                                ...
                                <div className="grid grid-cols-2 gap-12 mb-12">
                                    <div>
                                        ...
                                        <div className="space-y-1">
                                            <p className="font-bold text-blue-600 text-lg leading-none mb-1">{vendor.displayName || vendor.companyName || vendor.name}</p>
                                            {formatAddress(vendor.billingAddress).map((part: any, i: any) => <p key={i} className="text-sm text-slate-600">{part}</p>)}
                                        </div>
                                    </div>
                                    <div className="bg-slate-50 p-6 rounded-sm border border-slate-100">
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div className="flex justify-between text-sm"><span className="text-slate-500">Billed Amount</span><span className="font-medium">{formatCurrency(vendor.balanceToPay || vendor.outstandingPayables || 0)}</span></div>
                                            <div className="flex justify-between text-sm"><span className="text-slate-500">Amount Paid</span><span className="font-medium text-green-600">{formatCurrency(0)}</span></div>
                                            <div className="pt-3 border-t border-slate-200 flex justify-between"><span className="font-bold uppercase text-xs">Balance Due</span><span className="font-bold text-lg">{formatCurrency(vendor.balanceToPay || vendor.outstandingPayables || 0)}</span></div>
                                        </div>
                                    </div>
                                </div>
                                        ...
                                        <tbody className="divide-y divide-slate-100">
                                            {statementTransactions.length === 0 ? (
                                                ...
                                                statementTransactions.map((tx: any) => (
                                                    <tr key={tx.id}>
                                                        <td className="px-4 py-3 text-xs">{format(new Date(tx.billDate || tx.date), "dd/MM/yyyy")}</td>
                                                        <td className="px-4 py-3"><p className="text-xs font-bold text-blue-600">{tx.billNumber}</p><p className="text-[10px] text-slate-400">Bill</p></td>
