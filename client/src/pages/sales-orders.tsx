...
function SalesOrderDetailPanel({ order, branding, organization, onClose, onEdit, onDelete, onConvertToInvoice, onConvertSelectedItems }: SalesOrderDetailPanelProps & { branding?: any; organization?: any }) {
  ...
  const handleSendToCustomer = async () => {
    try {
      ...
      if (result.success) {
        toast({ title: "Success", description: "Sales order sent to customer successfully" });
        onEdit(); // Trigger refresh
      } else {
        throw new Error(result.message);
      }
    } catch (error: any) {
      toast({ title: "Error", description: error.message || "Failed to send sales order", variant: "destructive" });
    }
  };
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-56 font-display">
            <DropdownMenuItem
              onClick={async () => {
                try {
                  const response = await fetch(`/api/flow/sales-orders/${order.id}/generate-invoice`, {
                    method: "POST",
      ...
      {showPdfPreview ? (
        <div className="flex-1 overflow-auto scrollbar-hide bg-slate-100 dark:bg-slate-800 p-8 flex justify-center">
          <div className="w-full max-w-[210mm] shadow-lg bg-white dark:bg-white">
            <SalesOrderPdfView order={order} branding={branding} organization={organization} />
          </div>
        </div>
      ) : (
